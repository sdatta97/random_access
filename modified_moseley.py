...
V0 = [0.0, 0.0]
V1 = [[0.0] * 961 for _ in range(2)]
V2 = [[[0.0] * 1481 for _ in range(1481)] for _ in range(2)]
V11 = [[[0.0] * 961 for _ in range(961)] for _ in range(2)]
V12 = [[[0.0] * 961 for _ in range(961)] for _ in range(2)]
alph = [0.0] * 961
beta = [[0.0] * 1481 for _ in range(1481)]
NUM_ITERATIONS = 50
vmax = 0.0
tmax1 = 0.99
tex = 0.0
...
for n in range(1, NUM_ITERATIONS + 1):
    ...
    for l in range(1, 20):
        ...
        dif1 = 1 - dif
        ia = ia * m1 + 1
        vn = ea * (1 + V0[i]) + a * ea * (1 + V1[j][0]) + (1 - ea - a * ea) * V12[j][0][ia + m1 - 1] * adif
        if ia == 1:
            vn = vn + (1 - ea - a * ea) * V12[j][0][0] * adif1
        if ia > 1:
            vn = vn + (1 - ea - a * ea) * V12[j][0][ia - 1] * adif1
        if vn > vmax:
            amax = a
            vmax = vn
    alph[0] = amax
    ...
    if xmin < 0:
        print("Neg xmin: %f" % xmin)
    for k in range(m2, 962, m1):
        x = (k - 1) * del_
        ex = math.exp(-x)
        for l in range(1, 962 - k, m1):
            y = (l - 1) * del_
            ey = math.exp(-y)
            exy = ex * ey
            b = 0.0
