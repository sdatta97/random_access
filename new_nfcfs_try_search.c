...
int main() 
	{
		...
		for(n=1; n<= NUM_ITERATIONS; n++) { 						// Main iteration loop 
			...
			//Calculate Vsub1[0][y], V122[0][y][delta], V22[0][y], V11[0][y] and V12[0][y], y>0
			...
			//Calculate Vsub1[x][0], V122[x][0][0][0], V22[x][0], V11[x][0] and V12[x][0], x>0
			for(l = m2; l<=97; l = l+m1) {
				x =(l-1)*del;
				ex = exp(-x);
				...
				x = V22[i][l-1][0] - V22[j][l-1][0];					// Update change in value for state
			
				if(x<xmin) 	xmin = x;
				if(x>xmax) 	xmax = x;
				if (xmin < 0) {
					printf("V22[%d][0]: Neg xmin: %f\n", l-1, xmin);
				}
			...
			//Calculate Vsub1[x][y], V122[x][y][delta], V22[x][y], V11[x][y] and V12[x][y]
			for(l=m2; l<=97; l=l+m1) {		
				x = (l-1)*del;
				ex = exp(-x);
				ex2 = exp(-0.5*x);
				for(la = m2; la<=97-l; la = la+m1) {
					...
					xy = V22[i][k-1][l-1] - V22[j][k-1][l-1];					// Update change in value for state
					if(xy<xmin) xmin = xy;
					if(xy>xmax) xmax = xy;
					if (xmin < 0) {
						printf("curr V22 = %f, prev V22 = %f, k = %d, l = %d\n", V22[i][k-1][l-1], V22[j][k-1][l-1], k, l);
						printf("Neg xmin: %f\n", xmin);
					}
					// V22[i][l-1][la-1] = (ex2*(1-(1+x/2)*ex2)*V22[j][(l-1)/2][la-1]+0.5*x*ex2*(1-ex2)*(1+V12[j][(l-1)/2][la-1])+(1-(1+0.5*x)*ex2)*V22[j][(l-1)/2][la-1])/(1-(1+x)*ex);
					// V12[i][l-1][la-1] = (x*ex*eax2*(1-(1+0.5*(a-x))*eax2)*(1+V2[j][(la-1)/2][0])+ (x*ex*(1-eax2-0.5*(a-x)*eax)+(1-(1+x)*ex)*(1-(1+(a-x))*eax))*V122[j][l-1][la-1])/((1-ex)*(1-(1+a-x)*eax));						// From value expression VS12(x)
