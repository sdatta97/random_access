...
int main() 
	{
		double  V0[2], V1[2][193], V2[2][295][295];					// Initilization of value arrays
		...
		double b1max, bxdif, bxdif1, exy, bmax, bmax1, xy, ex, exb, exyb, axdif, axdif1, a, ea, amax1, amax, adif, adif1, b, y, ey, eb, eyb, bdif, bdif1;
		...
		for(n=1; n<= NUM_ITERATIONS; n++) { 						// Main iteration loop 
			...
			for(l=m2; l<=193; l=l+m1){
				...
				for(b=delf1;b<y;b=b+delf1){
					...
					*/
					...
					if(ib==1) {
						vn = (vn + (1-eb)*(1+V1[i][0])*bdif1)/(1-ey);				// Substituting value expression lim y -> 0 VS2(0,y) with linear interpolation
						//vn = vn + (eyb - ey)*0.5*(1+V1[i][0] + V2[i][0][0])*dif1;				// Substituting value expression lim y -> 0 VS2(0,y) with linear interpolation
						//vn = (vn + (eyb - ey)*(1+V1[i][0])*dif1 + (1-(1+y-b)*eyb)*(1+V1[i][0])*dif1)/(1-ey);				// Substituting value expression lim y -> 0 VS2(0,y) with linear interpolation
					}
			...
			for(l=1; l<=19; l++) {
				...
				vn = ea*(1+V0[j]) + (1-ea)*V2[j][0][ia + m1 - 1]*adif;		// From value expression VS1(0) and interpolation
			
				if(ia==1) {
					vn = vn + (1-ea)*(1 + V1[i][0])*adif1;					// Substituting value expression lim y -> 0 VS2(0,y) with linear interpolation 
				    //vn = vn + (1-ea)*0.5*(1+V1[i][0] + V2[i][0][0])*adif1;					// Substituting value expression lim y -> 0 VS2(0,y) with linear interpolation 
				}
			
				if(ia>1) {
					vn = vn + (1-ea)*V2[j][0][ia-1]*adif1;					// From value expression VS1(0) and interpolation
				}
			...
			for(l=m2; l<=193; l=l+m1) {
				...
				for(la = m2; la<=193; la = la+m1) {
					...
					if(a>=x) {
						vn = x*ea*(1 + V0[j]) + (1-ex-x*ea)*V2[j][l-1][la-l];						// From value expression VS1(x)
					}
				...
				for(la = 1; la<=19; la++) {
					...
					if(a>x) {
						...
						dif1 = 1 - dif;
						ia = ia*m1 + 1;
						vn = x*ea*(1+V0[j]) + (1-ex-x*ea)*(V2[j][l-1][ia-1]*dif1 + V2[j][l-1][ia+m1-1]*dif);		// From value expression VS1(x) and interpolation
					}
			...
			//Calculate V2[x][y] a	nd B[x][y], x>0
	    ...
	    FILE *out_file1  = fopen("out_moseley_2.txt", "w"); // read only          
