...
int main() 
	{
		...
		double exy, xy, ex,axdif, axdif1, a, ea, ea2, b,eb,exb, exyb, eax, eax2, amax1, amax, bmax, b1max, adif, adif1, bdif,bdif1, y, ey,bxdif,bxdif1;
		
		int i, j, k, l, m, n, it, lb, ia, ib, iax, la, k1, kl,ibx, ibxy;
		...
		for(n=1; n<= NUM_ITERATIONS; n++) { 						// Main iteration loop 
			...
			for(l=m2; l<=193; l=l+m1){
				...
				y = (l-1)*del;
				ey = exp(-y);
                vn = (y*ey*(1 + V1[j][0]) + (1 - (1+y)*ey)*V12[j][0][l-1])/(1-ey);				// Substituting value expression lim y -> 0 VS2(0,y) with linear interpolation
				
				V2[i][0][l-1] = vn;  //vmax;								// Update V2 for next iteration
			...
			for(l=m2; l<=193; l=l+m1) {
				...
				for(la = m2; la<=193; la = la+m1) {
					...
					if(a>x) {
						vn = x*ea*(1 + V0[j]) + (1-ex-x*ea)*V2[j][l-1][la-l];	
                    }
	    ...
	    FILE *out_file_new  = fopen("out_russian_new.txt", "w"); // read only 
        // test for files not existing. 
   	    if (out_file_new == NULL) 
        {   
            printf("Error! Could not open file\n"); 
            exit(-1); // must include stdlib.h 
        } 
		for(i=0;i<=192;i++) {
			fprintf(out_file_new,"%lf, %lf, %lf\n", i*del, V1[1][i], alph[i]);
		}
